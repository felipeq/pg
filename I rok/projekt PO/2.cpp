...
{	
		int x=4;
		if(vEnemy[i].posx<vEnemy[i].targetX) vEnemy[i].posx+=(vEnemy[i].speed*delta*0.75)*x;
		if(vEnemy[i].posx>vEnemy[i].targetX) vEnemy[i].posx-=(vEnemy[i].speed*delta)*x;
		if(vEnemy[i].posy<vEnemy[i].targetY) vEnemy[i].posy+=(vEnemy[i].speed*delta*0.75)*x;
		if(vEnemy[i].posy>vEnemy[i].targetY) vEnemy[i].posy-=(vEnemy[i].speed*delta)*x;	
}
...
void gracz::atakuj(int numer)
{
	...
	{
		case 1:
			vAttack.push_back(kula_Iron(*this));
			break;
	}
}
